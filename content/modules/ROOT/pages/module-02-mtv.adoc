...
. 移行元のソースプロバイダを選択するよう求められます。Plan nameを入力し、*VMware* のタイルをクリックします。
* *Plan name*: move-webapp-vmware
+
image::2025_spring/module-02-mtv/Mod_08_VMware_Source_Provider.png[link=self, window=blank, width=100%]
...
+
マイグレーションプランに以下の値を入力し、*Create migration plan* ボタンをクリックします。
...
+
image::2025_spring/module-02-mtv/Mod_11_Create_Migration_Plan.png[link=self, window=blank, width=100%]
...
. しばらくすると、プランが *Ready* になります。*Start* ボタンをクリックして、移行プロセスを開始します。
+
